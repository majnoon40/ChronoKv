// chrono_kv_v8_oracle.cpp — missed-read oracle for v8 (lock-free registration).
//
// commit logs every (ts, value) per key before the completion flag, so the log
// holds every version a snapshot could entitle. read() checks its result against
// the newest logged version <= its committed snapshot r. A mismatch = a missed
// read (reclamation failure).
//
// Positive control (-DBROKEN_GC): GC threshold drops the published_ bound
// (m = reg_min only, the v3 bug). With no registered readers m = inf, so GC
// truncates aggressively and a later reader misses its entitled version. The
// oracle must fire (violations > 0), proving the instrument is live.
//
// Build & run:
//   g++ -std=c++17 -O0 -g -DWIDEN -pthread chrono_kv_v8_oracle.cpp -o v8_oracle
//   g++ -std=c++17 -O0 -g -DWIDEN -DBROKEN_GC -pthread chrono_kv_v8_oracle.cpp -o v8_oracle_broken
//   ./v8_oracle_broken   # expect: oracle violations > 0  (CONTROL FIRED)
//   ./v8_oracle          # expect: oracle violations: 0   (v8 LOGIC CLEAN)
#include <atomic>
#include <mutex>
...
#ifdef BROKEN_GC
    // v3 bug: reg_min only, no published_ bound.
    uint64_t rm = UINT64_MAX;
    for (size_t i = 0; i < MAX_READERS; ++i) {
      uint64_t v = slots_[i].load(memory_order_acquire);
      if (v != 0) rm = min(rm, v - 1);
    }
    return rm;
#else
    // Correct: published_ FIRST, then slots; m = min(reg_min, published_).
...
public:
  ...
  void commit(const string& key, const string& value) {
    ...
    uint64_t ts;
    {
      ...
      WIDEN_YIELD();
      {
        lock_guard<mutex> olk(oracle_mu_);          // oracle: log before visible
        oracle_log_[idx].push_back({ts, value});
      }
  ...
  optional<string> read(const string& key) {
    ...
    uint64_t r;
    for (;;) {
      r = published_.load(memory_order_seq_cst);            // sample
      WIDEN_YIELD();
      slots_[slot].store(r + 1, memory_order_release);      // register
      uint64_t r2 = published_.load(memory_order_seq_cst);  // double-check
      if (r2 == r) break;
    }
    ...
    slots_[slot].store(0, memory_order_release);            // unregister
    ...
    if (idx >= 0) {
      ...
      for (auto rit = log.rbegin(); rit != log.rend(); ++rit) {
        if (rit->first <= r) { expected = rit->second; break; }
      }
    }
    if (result != expected) violations_.fetch_add(1);
    return result;
  }
...
#ifdef BROKEN_GC
  cout << (v > 0 ? "CONTROL FIRED (expected) — instrument is live\n"
                 : "CONTROL DID NOT FIRE — instrument broken; do NOT trust a clean v8\n");
#else
  cout << (v == 0 ? "v8 LOGIC CLEAN (reads returned their entitled version every time)\n"
                  : "v8 LOGIC VIOLATIONS — missed reads detected\n");
#endif
  return 0;
}
