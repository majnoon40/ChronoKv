// chrono_kv_v9_oracle.cpp — missed-read oracle for v9 (v8 + overflow log).
//
// commit logs ground truth; read checks its result against the newest committed
// version <= its snapshot r. The overflow log rides along in commit but doesn't
// touch the read path's logic, so correct v9 -> 0 violations, and the
// no-published-bound GC control must still fire (it truncates a reader's needed
// version during the sample->register gap, which the double-check can't catch
// because truncation doesn't move published_).
//
// Build & run:
//   g++ -std=c++17 -O0 -g -DWIDEN -DBROKEN_GC -pthread chrono_kv_v9_oracle.cpp -o v9_oracle_broken
//   g++ -std=c++17 -O0 -g -DWIDEN -pthread chrono_kv_v9_oracle.cpp -o v9_oracle
//   ./v9_oracle_broken   # expect: violations > 0 (CONTROL FIRED)
//   ./v9_oracle          # expect: violations: 0 (v9 LOGIC CLEAN)
#include <atomic>
#include <mutex>
...
int main() {
  ...
  cout << (v > 0 ? "CONTROL FIRED (expected) — instrument is live\n"
                 : "CONTROL DID NOT FIRE — instrument broken; do NOT trust a clean v9\n");
#else
  cout << (v == 0 ? "v9 LOGIC CLEAN (reads returned their entitled version every time)\n"
                  : "v9 LOGIC VIOLATIONS — missed reads detected\n");
#endif
  return 0;
}
